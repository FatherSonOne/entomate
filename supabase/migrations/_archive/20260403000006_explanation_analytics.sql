...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own analytics' AND tablename = 'explanation_analytics') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service can insert analytics' AND tablename = 'explanation_analytics') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service can read all analytics' AND tablename = 'explanation_analytics') THEN
...
-- Verification query
SELECT 'Explanation analytics table created successfully' AS status;
