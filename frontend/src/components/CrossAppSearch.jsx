  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto">
      ...
      <div className="flex min-h-full items-start justify-center p-4 pt-[10vh]">
        <div
          className="relative w-full max-w-2xl transform rounded-xl bg-surface shadow-2xl ring-1 ring-black/5 transition-all"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="flex items-center border-b border-line-default px-4">
            <Search className="w-5 h-5 text-content-tertiary" aria-hidden="true" />
            <input
              ref={inputRef}
              type="text"
              className="flex-1 border-0 bg-transparent py-4 px-3 text-content-primary placeholder-gray-400 focus:outline-none focus:ring-0 text-lg"
            ...
            <button
              onClick={onClose}
              className="rounded p-1 text-content-tertiary hover:text-content-secondary ml-2"
          ...
          <div ref={listRef} className="max-h-[60vh] overflow-y-auto">
            ...
            {!loading && !error && results.length === 0 && query.length < 2 && (
              <div className="p-4">
                {recentSearches.length > 0 && (
                  <div className="mb-4">
                    <h3 className="text-xs font-medium text-content-tertiary uppercase tracking-wide mb-2 px-2">
                      Recent Searches
                    </h3>
                    <div className="space-y-1">
                      {recentSearches.slice(0, 5).map((recent, idx) => (
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left hover:bg-surface-muted transition-colors"
                        >
                          <Clock className="w-4 h-4 text-content-tertiary" />
                          <span className="text-content-secondary">{recent.query}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <div className="text-center py-6 text-content-tertiary">
                  <Search className="w-12 h-12 text-content-muted mx-auto mb-3" />
                  <p className="text-sm">
                    Search across <span className="font-medium text-content-secondary">contacts</span>,{' '}
                    <span className="font-medium text-content-secondary">meetings</span>,{' '}
                    <span className="font-medium text-content-secondary">deals</span>, and more
                  </p>
            ...
            {!loading && !error && results.length === 0 && query.length >= 2 && (
              <div className="p-8 text-center">
                <Search className="w-12 h-12 text-content-muted mx-auto mb-3" />
                <p className="text-content-secondary font-medium">No results found</p>
                ...
                {grouped.map((group) => {
                  ...
                  return (
                    <div key={group.type} className="mb-3">
                      {/* Group header */}
                      <div className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-content-tertiary uppercase tracking-wide">
                        <TypeIcon className="w-4 h-4" style={{ color: group.typeInfo?.color }} />
                        {group.typeInfo?.label || group.type}
                        <span className="text-content-tertiary">({group.items.length})</span>
                      </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {result.description && (
                                <p className="text-xs text-content-tertiary line-clamp-1 mt-0.5">
                                  {result.description}
                                </p>
                              )}
                            </div>
          ...
          {/* Footer */}
          <div className="flex items-center justify-between px-4 py-2.5 border-t border-line-default bg-surface-muted rounded-b-xl">
            <div className="flex items-center gap-4 text-xs text-content-tertiary">
              <span className="flex items-center gap-1">
                <kbd className="px-1.5 py-0.5 bg-surface rounded border border-line-default font-mono">
                  ↑↓
                </kbd>
                navigate
              </span>
