...
type ShortcutMap = Record<string, ShortcutCallback | ShortcutConfig>;

/**
 * Custom hook for handling keyboard shortcuts
 * @param shortcuts - Object mapping shortcut keys to callbacks
...
export function useKeyboardShortcuts(shortcuts: ShortcutMap, enabled: boolean = true) {
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    for (const [shortcut, config] of Object.entries(shortcuts)) {
      const { callback, allowInInput = false } =
        typeof config === 'function' ? { callback: config, allowInInput: false } : config;
